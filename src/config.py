...
_table = ["none", "monostable", "bistable", "monostableNC", "bistableNC"]
timeOnOff = {}
gpio = const("gpio")
mesh = None
...
def conf():
    ...
    mesh = "ihome/{}".format(setup.name or uid)
    return {
        "led": 255,
        "label": setup.label,
...
def initPin(p1, tp):
    ...
    try:
        ...
        if not p in dados[PINS].keys():
            r = Pin(int(x), Pin.IN)
            if tp == PININ:
                r.irq(trigger=Pin.IRQ_RISING, handler=interruptEvent)
            dados[PINS][p] = r
